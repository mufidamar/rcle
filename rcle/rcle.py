...
# -*- coding: utf-8 -*-
import argparse
from .utils import RCLE
from .helpers import *

rcle = RCLE()
choices = rcle.get_app_names()
choices.append('all')
...
def main():
	...
	if args.install:
		target = args.install
		if target.lower() == 'all':
			apps = rcle.get_app_names()
			if len(apps) > 0:
				for app in apps:
					if not rcle.has_ssl(app):
						print_message('Processing app {}'.format(app), 'INFO')
						try:
							rcle.install_ssl(app)
